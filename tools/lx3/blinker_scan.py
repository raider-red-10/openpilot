...
meaningless.
  ...
  scan:  ./blinker_scan.py                  (parked, car on, follow the prompts)
  watch: ./blinker_scan.py 0 0x1b0 20 1     (live value of bus 0, 0x1b0, byte 20, bit 1)
"""
import sys
import time
...
PHASES = [
  ...
  ("off1", "blinkers OFF -- do not touch anything", 8),
  ("right", "RIGHT blinker ON -- push the stalk so it LOCKS on", 12),
  ("off2", "blinkers OFF -- do not touch anything", 8),
]

# Phases with no turn signal active. A blinker bit must be static in all of them.
...
def main():
  ...
  lines.append(f"PAIRED -- flashes for both stalks, static when off ({len(paired)} messages)")
  for k in paired:
    bus, addr = k
    w = WIDTH.get(k, 8)
    ...
    lines.append(f"  bus={bus} addr=0x{addr:03x}  LEFT[{', '.join(lb)}]  RIGHT[{', '.join(rb)}]"
                 f"   ~{rate:.1f} changes/s")
  ...
  with open(OUT, "w") as f:
    f.write(out + "\n")
  print(f"\nwritten to {OUT}")
  print("confirm a candidate live, e.g.:  ./blinker_scan.py 0 0x1b0 20 1")
